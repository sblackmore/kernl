...
pub fn handle_request(mut request: Request, storage: &Arc<Storage>) {
    ...
    let (status, content_type, response_body) = route(&method, &url, &body, storage);
...
pub fn route(
    ...
    match (method, segments.as_slice()) {
        ...
        ("POST", ["api", "v1", "packages"]) => handle_publish(body, storage),
...
mod tests {
    ...
    fn test_route_not_found() {
        ...
        let (status, _, body) = route("GET", "/nonexistent", &[], &storage);
    ...
    fn test_route_health() {
        let (storage, _dir) = test_storage();
        let (status, _, body) = route("GET", "/health", &[], &storage);
        assert_eq!(status.0, 200);
    ...
    fn test_publish_via_route() {
        ...
        let (status, _, resp) = route("POST", "/api/v1/packages", &body, &storage);
